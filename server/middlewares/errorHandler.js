...
const classify = (err) => {
    ...
    if (status < 500) {
        ...
        // An error message is written for a developer and often carries a
        // connection string, path or field name.
...
const errorHandler = (err, req, res, next) => {
    ...
    // A response already went out; Express's default handler closes the
    // connection rather than throwing again.
    if (res.headersSent) {
        return next(err);
    }
